<template>
  <div id="app">
    ...
    <div class="postwapper">
      <div class="container">
        <div class="row">
          <div
            class="col s4 m4 mt-4"
...
export default {
  data() {
    return {
      ...
      postsUrl: "http://localhost/wordpress_vue/wp-json/wp/v2/posts",
      queryOptions: {
        per_page: 9,
        page: 1,
        _embed: true,
    ...
    handleClickLoadMore() {
      ...
      axios
        ...
        .then((response) => {
          ...
          // this.posts  = [].concat.apply(JSON.parse(JSON.stringify((res.data))), this.posts);
          if (response.data) {
            response.data.length < 9 ? (this.disable = true) : "";
            response.data.map((i) => {
              this.posts.push(i);
            });
          }
